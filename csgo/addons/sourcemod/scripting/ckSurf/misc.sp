...
stock void PrintChatStage (int client, int zGroup, int rank = 0)
{
		...
		(rank <= GetConVarInt(g_hAnnounceRank) || GetConVarInt(g_hAnnounceRank) == 0))
	{
		...
		{
			RecordDiff = g_fOldStageRecordTime[zGroup] - g_fFinalTime[client];
			FormatTimeFloat(client, RecordDiff, 3, szRecordDiff, 54);
			Format(szRecordDiff, 54, "-%s", szRecordDiff);
		}
		...
		if (g_stageFirstRecord[client] && g_stageSRVRecord[client])
		{
			...
			if (g_tmpStageCount[zGroup] == 0)
				PrintToChatAll("%t", "StageFinished3", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, ORANGE, szZoneGroupName, GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, WHITE, LIMEGREEN, g_szFinalTimeStage[client], WHITE);
			else
				PrintToChatAll("%t", "StageFinished4", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, ORANGE, szZoneGroupName, GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, szRecordDiff, GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szFinalTimeStage[client], WHITE);
		}
		if (g_stagePBRecord[client] && g_stageSRVRecord[client])
		{
			PrintToChatAll("%t", "StageFinished2", MOSSGREEN, WHITE, LIMEGREEN, szName, ORANGE, szZoneGroupName, GRAY);
			PrintToChatAll("%t", "StageFinished5", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, ORANGE, szZoneGroupName, GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, szRecordDiff, GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szFinalTimeStage[client], WHITE);
		}
		if (g_stagePBRecord[client] && !g_stageSRVRecord[client])
		{
			PlayUnstoppableSound(client);
			PrintToChat(client, "%t", "StageFinished6", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, ORANGE, szZoneGroupName, GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, g_szStageTimeDifference[client], GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szStageFastestTime[zGroup], WHITE);
		}
		if (g_stageFirstRecord[client] && !g_stageSRVRecord[client])
		{
			PrintToChat(client, "%t", "StageFinished7", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, ORANGE, szZoneGroupName, GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szStageFastestTime[zGroup], WHITE);
		}
		if (!g_stageSRVRecord[client] && !g_stageFirstRecord[client] && !g_stagePBRecord[client])
		{
 			PrintToChat(client, "%t", "StageFinished1", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, ORANGE, szZoneGroupName, GRAY, RED, g_szFinalTimeStage[client], GRAY, RED, g_szStageTimeDifference[client], GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szStageFastestTime[zGroup], GRAY);
		}
	}
	else
	{
		...
		if (g_stageFirstRecord[client] && g_stageSRVRecord[client])
		{
			...
			if (g_tmpStageCount[zGroup] == 0)
				PrintToChat(client, "%t", "StageFinished3", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, YELLOW, g_szZoneGroupName[zGroup], GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, WHITE, LIMEGREEN, g_szFinalTimeStage[client], WHITE);
			else
				PrintToChat(client, "%t", "StageFinished4", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, YELLOW, g_szZoneGroupName[zGroup], GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, szRecordDiff, GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szFinalTimeStage[client], WHITE);
		}
		if (g_stagePBRecord[client] && g_stageSRVRecord[client])
		{
			PrintToChat(client, "%t", "StageFinished2", MOSSGREEN, WHITE, LIMEGREEN, szName, YELLOW, g_szZoneGroupName[zGroup]);
			PrintToChat(client, "%t", "StageFinished5", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, YELLOW, g_szZoneGroupName[zGroup], GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, szRecordDiff, GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szFinalTimeStage[client], WHITE);
		}
		if (g_stagePBRecord[client] && !g_stageSRVRecord[client])
		{
			PlayUnstoppableSound(client);
		...
		if (g_stageFirstRecord[client] && !g_stageSRVRecord[client])
		{
			PrintToChat(client, "%t", "StageFinished7", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, YELLOW, g_szZoneGroupName[zGroup], GRAY, LIMEGREEN, g_szFinalTimeStage[client], GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szStageFastestTime[zGroup], WHITE);
		}
		if (!g_stageSRVRecord[client] && !g_stageFirstRecord[client] && !g_stagePBRecord[client])
		{
			if (IsValidClient(client))
	 			PrintToChat(client, "%t", "StageFinished1", MOSSGREEN, WHITE, LIMEGREEN, szName, GRAY, YELLOW, g_szZoneGroupName[zGroup], GRAY, RED, g_szFinalTimeStage[client], GRAY, RED, g_szStageTimeDifference[client], GRAY, LIMEGREEN, g_MapRankStage[zGroup][client], GRAY, g_iStageCount[g_doingStage[client]], LIMEGREEN, g_szStageFastestTime[zGroup], GRAY);
		}
	...
	Call_PushCell(client);
	Call_PushFloat(g_fFinalTime[client]);
	Call_PushString(g_szFinalTimeStage[client]);
	Call_PushCell(rank);
	Call_PushCell(g_iStageCount[zGroup]);
	Call_PushCell(zGroup);
	
	/* Finish the call, get the result */
	Call_Finish();
	
	CheckStageRanks(client, zGroup);
...
public void CheckStageRanks(int client, int zGroup)
{
	// if client has risen in rank,
	if (g_OldMapRankStage[zGroup][client] > g_MapRankStage[zGroup][client])
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsValidClient(i) && !IsFakeClient(i) && i != client)
			{  //if clients rank used to be bigger than i's, 2nd: clients new rank is at least as big as i's
				if (g_OldMapRankStage[zGroup][client] > g_MapRankStage[zGroup][i] && g_MapRankStage[zGroup][client] <= g_MapRankStage[zGroup][i])
					g_MapRankStage[zGroup][i]++;
			}
		}
	}
}
