...
function calculatePrice(){
	...
	var totalBruteVal = (weekRevenue + weekEndRevenue)
	var uberFare = (weekRevenue + weekEndRevenue)*0.25
	var spentInFuel = ( weekKm + weekEndKm ) * pricePerKmFuel

	var totalPriceWeek = (totalBruteVal) - (spentInFuel) - (uberFare).toFixed(0)

	var $spentFuelHtml = $('#element-521').children()
	var totalKms = weekKm + weekEndKm

	var totalPriceMonth = totalPriceWeek

	var $totalNetoHtml = $('#element-533').children()
	var $uberFareHtml = $('#element-539').children()
	var $totalBrute = $('#element-541').children()


	while ($totalNetoHtml.length){
	  $totalNetoHtml = $totalNetoHtml.children();
	}
	while( $spentFuelHtml.length ) {
	  $spentFuelHtml = $spentFuelHtml.children();
	}
	while( $totalBrute.length ) {
	  $totalBrute = $totalBrute.children();
	}
	while( $uberFareHtml.length ) {
	  $uberFareHtml = $uberFareHtml.children();
	}



	gradualIncrease($totalNetoHtml.end(), totalPriceMonth, 750)

	gradualIncrease($spentFuelHtml.end(), spentInFuel, 1200)
	gradualIncrease($uberFareHtml.end(), uberFare, 1200)
	gradualIncrease($totalBrute.end(), totalBruteVal, 1750)
...
// MAndar multiplos de 10 en speed.
function gradualIncrease(htmlInput, numberToReach, speed){
	var number = 0;
